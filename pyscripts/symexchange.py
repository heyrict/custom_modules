...
from data_processing import auto_newline, split_wrd
...
def main():
    ...
    opt.add_option(
        '-u', '--to-full', dest='to_full', action='store_true', default=False)
    ...
    if options.only:
        ...
        for i in options.exclude:
            try:
                HALFFULL.remove(tuple(i))
            except:
                print('WARNING:', i, 'is not in preset cases')
